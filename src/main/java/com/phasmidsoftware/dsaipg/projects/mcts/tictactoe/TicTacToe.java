...
public class TicTacToe implements Game<TicTacToe> {
    ...
    State<TicTacToe> runGameWithAI() {
        ...
        while (!state.isTerminal()) {
            ...
            Node<TicTacToe> bestNode = mcts.getBestMove();
            // Node<TicTacToe> bestNode = root.bestChild(); // Use UCB to select the best child
    ...
    private static State<TicTacToe> humanMove(State<TicTacToe> state) {
        while (true) {
            ...
            try {
                ...
                return state.next(new TicTacToeMove(state.player(), row, col));
    ...
    State<TicTacToe> runGameInteractive() {
        State<TicTacToe> state = start();
        state.render();
        int player = opener();
        while (!state.isTerminal()) {
            if (player == X) {
                ...
                // mcts.printBestPath(root, 2147483647); // debug
    ...
    class TicTacToeState implements State<TicTacToe> {
        ...
        public Move<TicTacToe> chooseMove(int player) {
            Collection<Move<TicTacToe>> moves = moves(player);
            if (moves.isEmpty()) throw new RuntimeException("no moves available for player " + player);
