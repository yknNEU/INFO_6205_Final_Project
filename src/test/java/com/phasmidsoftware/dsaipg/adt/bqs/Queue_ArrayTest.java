...
public class Queue_ArrayTest {
    ...
    @Test
    public void offer3() {
        Integer[] Integers = new Integer[]{10, 11, 12, null, null}; // Pre-filled queue for wrap-around testing
        Queue_Array<Integer> queue = new Queue_Array<>(Integers, 0, 3);
        queue.offer(13); // Adding an item causes wrap-around
        queue.offer(14);
        queue.show();
        Object[] array = queue.asArray();
        assertArrayEquals(new Object[]{10, 11, 12, 13, 14}, array); // Full array includes wrapped values
    }

    @Test
    public void offer4() {
        Queue_Array<Integer> queue = new Queue_Array<>(3); // Start with a small queue to trigger resizing
        queue.offer(1);
        queue.offer(2);
        queue.offer(3);
        queue.offer(4); // This should trigger the resize
        queue.offer(5);
        queue.show();
        assertEquals(5, queue.size());
        Object[] array = queue.asArray();
        assertArrayEquals(new Object[]{1, 2, 3, 4, 5}, array); // Ensure all items are correctly retained
    }
}